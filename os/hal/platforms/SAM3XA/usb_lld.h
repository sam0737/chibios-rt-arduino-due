...
/**
 * @file    SAM3XA/usb_lld.h
 * @brief   SAM3XA USB Driver subsystem low level driver header.
 *
 * @addtogroup USB
 * @{
 */
...
/**
 * @brief   USB default IRQ priority
 */
#define SAM3XA_USB_DEFAULT_IRQ_PRIORITY     12
 ...
 */
#if !defined(SAM3XA_USB_USE_UOTGHS) || defined(__DOXYGEN__)
#define SAM3XA_USB_USE_UOTGHS             FALSE
#endif
/** @} */
...
typedef struct {
  ...
  /**
   * @brief   Number of bank to use (ignored for CTRL endpoint)
   * @note    Zero for default (use one bank)
   */
  uint8_t                       bank;
} USBEndpointConfig;
...
struct USBDriver {
   ...
   */
  size_t                    ep_size[USB_MAX_ENDPOINTS + 1];
};
